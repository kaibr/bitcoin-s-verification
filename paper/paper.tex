...
\paragraph{The bitcoin-s Library.} Here, we try to verify two
properties of the bitcoin-s library
\cite{BitcoinS:website,BitcoinS:github}, which implements parts of the
Bitcoin protocol in Scala. In particular, it allows to serialize,
deserialize, sign and validate transactions. The library uses
immutable data structures and algebraic data types but is not written
with formal verification in mind. According to the website, the
library is used in production, ``handling millions of dollars each
day'' \cite{BitcoinS:website}.
