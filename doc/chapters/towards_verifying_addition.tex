...
Thus, the constructor parameter of BitInt must be a literal arguments.
...
This would be really hard to refactor, because Bitcoin-S tries to be as much dynamic as possible so it can be used with other cryptocurrencies than bitcoins.
Maybe it would even be impossible, because they need to parse dynamic values from the bitcoin network.

Luckily, we can use toBigInt on underlying directly instead of going over toLong.
So, instead of converting the underlying to Long and and back to BigInt we convert underlying directly to BigInt.
...
There are other number typs like UInt16 but for our property we don't need them.
...
This is not exact the same code, because an implementing class can not override A anymore but that's fine for our verification.
...
We can safely remove the string parameters from the requires, since they only serve as error message.
...
We can easily convert this Long value to a BigInt with string literal parameter by passing this numbers as strings to the BigInt constructor.
...
This show, that there is an invalid specification in checkResult and Stainless prints a counterexample for it.
